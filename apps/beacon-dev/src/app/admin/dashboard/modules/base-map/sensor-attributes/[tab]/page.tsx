...
export default function SensorAttributeTab({ params }: { params: Promise<{ tab: string }> }) {
  const { tab } = use(params);
  const sub = baseMapSubmodules.find(s => s.key === 'sensor-attributes');
  ...
  return (
    <div>
      <p style={{ fontSize: 12, color: '#9ca3af', marginBottom: 4 }}>Sensor-Driven Attributes</p>
