...
export default function UserLayerTab({ params }: { params: Promise<{ tab: string }> }) {
  const { tab } = use(params);
  const sub = baseMapSubmodules.find(s => s.key === 'user-layers');
  ...
  return (
    <div>
      <p style={{ fontSize: 12, color: '#9ca3af', marginBottom: 4 }}>User-Created Layers</p>
