...
- `C1.8c` — (local) — global volcano expansion: map now shows 165 volcanoes (30 US + 135 global from PVMBG, JMA, PHIVOLCS, INGV, IMO, KVERT, RVO, GNS, SERNAGEOMIN, IG-EPN, IGP, SGC, INSIVUMEH, CENAPRED, INETER, OVSICORI, MARN, OVG, OVPF, VMGD, MVO, UWI-SRC, OVSM, OVSG, IGN, INMG, TGS, SVERT, MEVO). Default view is global (lat/lng 15/30, zoom 2). Detail panel degrades gracefully for volcanoes without curated history/monitoring, with a "curated data is limited — use Smithsonian GVP" banner.
- `C1.8c.1` — (local) — Italian volcano curation: INGV-OV + INGV-OE monitoring entries for Campi Flegrei, Vesuvius, Etna, Stromboli, Vulcano (dense networks, seismic/GPS/tilt/strain/gas/thermal/webcam/infrasound/InSAR/hydrologic sensor details). Full Campi Flegrei eruption history (1538 Monte Nuovo, ~10ka Agnano-Monte Spina, ~15ka Neapolitan Yellow Tuff, ~39ka Campanian Ignimbrite), alt styles, 4 capability chips (bradyseism HIGH imminence — active crisis since 2005, phreatomagmatic moderate, VEI 7 none, hydrothermal blast moderate), baselines. Reworded sparse banner from "curated data is limited" to "Beacon hasn't curated this volcano yet" to make clear the gap is Beacon's, not the world's, and note that USGS FDSN global feed misses smaller volcanic quakes regional observatories track.
- `C1.8c.2` — (local) — regional quake feed router (pass 1): `/api/volcano-quakes` now dispatches by volcano location. `types.ts` + `fdsn-text.ts` + `router.ts` + `sources/{usgs,ingv,geonet,ipgp}.ts`. First regional sources wired: INGV (Italy — Campi Flegrei, Vesuvius, Etna, Stromboli, Vulcano, Ischia), GeoNet (NZ + Kermadec — Whakaari, Ruapehu, Taupo, Raoul), IPGP (Réunion, Martinique, Guadeloupe — Piton de la Fournaise, Pelée, La Soufrière). USGS remains the global fallback. Quakes section shows source name with operator link; sparse banner copy adapts based on whether a regional source is active. Replace-for-region strategy, M0+ floor. Pending: JMA, PVMBG, PHIVOLCS, KVERT, SERNAGEOMIN, IG-EPN, OVG, INSIVUMEH, CENAPRED, IMO, OVSICORI.
- `C1.8c.3` — (local) — quake feed router (pass 2): added EMSC source covering Aegean (Santorini, Nisyros, Methana, Milos), Azores (Pico, Fogo-Congro, Sete Cidades, Furnas, Faial), Canaries (Teide, Cumbre Vieja, El Hierro), Madeira. Also created `apps/beacon-dev/src/app/api/volcano-quakes/OBSERVATORIES.md` — honest roadmap listing every volcanic observatory globally with known/suspected endpoint URLs, implementation status (live/fdsn?/json?/html/blocked), and notes on quirks like BMKG vs PVMBG split, cross-border volcanoes, CORS. Sandbox cannot reach external endpoints to verify the `fdsn?` candidates (BMKG, NIED, CSN Chile, SGC, OVSICORI) — those need to be tried in a running dev server before committing source files for them.
...
**C1.8c — Global volcano expansion — DONE (local)**
- New `volcano-data-global.ts`: 135 globally significant volcanoes hand-curated from Smithsonian GVP + regional observatories, organised by region. Covers Indonesia (PVMBG) 19, Japan (JMA) 11, Philippines (PHIVOLCS) 6, Italy (INGV) 5, Iceland (IMO) 7, Greece 2, Kamchatka/Kurils (KVERT/SVERT) 8, PNG (RVO) 6, Vanuatu (VMGD) 5, New Zealand (GNS) 5, Africa 7, Chile (SERNAGEOMIN) 8, Ecuador (IG-EPN) 10, Peru (IGP) 3, Colombia (SGC) 5, Guatemala (INSIVUMEH) 4, Mexico (CENAPRED) 3, Nicaragua (INETER) 4, Costa Rica (OVSICORI) 4, El Salvador (MARN) 2, Antarctic (MEVO) 1, Caribbean 5, Atlantic (IGN/INMG) 3, Pacific (TGS) 2. Alert levels are conservative: persistently-active systems (Stromboli, Sakurajima, Yasur, Shiveluch, Fuego, Nyiragongo, Erebus, Sangay, Etna, Reventador, etc.) carry "watch" or "warning"; everything else is "normal" or "advisory".
- New `all-volcanoes.ts`: exports `ALL_VOLCANOES = [...VOLCANOES, ...VOLCANOES_GLOBAL]` — the combined list the map + recent-eruptions fly-out iterate over. Keeps the two source files independent for review.
- `volcano-playground.tsx`: swaps `VOLCANOES` → `ALL_VOLCANOES`, default map view changed from `[40, -130], zoom 4` (US only) to `[15, 30], zoom 2` (global).
- `get-recent-eruptions.ts`: swaps to `ALL_VOLCANOES`, adds a fallback path that parses `volcano.last_eruption` directly for volcanoes without a `VOLCANO_HISTORY` entry. Any volcano with `"present"` or `"persistent"` in its last-eruption label, or an `"warning"` alert level, or a 4-digit year within the window, now surfaces in the fly-out.
- `volcano-detail-panel.tsx`: new `sparse` flag (`!history && !monitoring`) renders a yellow banner between the location dropdown and the body — "Curated Beacon data for this volcano is limited. Live earthquake data will load from USGS FDSN below; for eruption history and monitoring details use the Smithsonian GVP link." Links out to `volcano.si.edu` with the volcano name. Keeps the experience honest for uncurated global entries instead of pretending data exists.
- `webcam-section.tsx`: now returns null when the volcano has no cams instead of rendering an empty "no webcams" card — cleaner for uncurated global volcanoes. Also updated the footer note to be observatory-agnostic.
- `playground.css` TWEAK ZONE 13 appended: `.vp-sparse-banner` (amber rounded panel) + `.vp-sparse-link`.
- Sandbox typecheck: 0 new errors, 28 baseline unchanged. 165 total volcanoes on the map (30 US + 135 global), no duplicate IDs.
- Verify: open the volcano playground → map now shows the full Ring of Fire, Africa, Europe, Antarctic instead of just the US. Click Etna → monitoring section is empty (not curated yet), the amber "curated data is limited" banner appears with a Smithsonian GVP link, and the USGS FDSN quakes section still loads real earthquakes around Etna's coordinates. Click Kilauea → full curated experience unchanged.
