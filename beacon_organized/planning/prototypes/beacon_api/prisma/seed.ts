...
async function main() {
  ...
  const superAdmin = await prisma.adminUser.upsert({
    ...
    create: {
      ...
      passwordHash: await hash("BeaconAdmin123!", 12),
      firstName: "Sarah",
      lastName: "Chen",
      role: "SUPER_ADMIN",
  ...
  const incidentCommander = await prisma.eMSUser.upsert({
    ...
    create: {
      ...
      passwordHash: await hash("EMSPassword123!", 12),
      firstName: "Michael",
      lastName: "Torres",
  ...
  const publicUser1 = await prisma.publicUser.upsert({
    ...
    create: {
      ...
      passwordHash: await hash("PublicUser123!", 12),
      firstName: "John",
      lastName: "Doe",
      displayName: "JohnD",
  ...
  const publicUser5 = await prisma.publicUser.upsert({
    ...
    create: {
      ...
      passwordHash: await hash("DemoUser123!", 12),
      firstName: "Demo",
      lastName: "User",
  ...
  const wildfireEvent = await prisma.event.create({
    data: {
      ...
      description:
        "Vegetation fire reported near River Road and Carmel Valley Road. Fire is wind-driven, moving northeast.",
      type: "WILDFIRE",
      severity: "SEVERE",
  ...
  const floodWatch = await prisma.event.create({
    data: {
      ...
      description:
        "Heavy rainfall expected over the next 48 hours. Salinas River may exceed flood stage in low-lying areas.",
      type: "FLOOD",
      severity: "MODERATE",
      status: "MONITORING",
      locationLat: 36.6696,
  ...
  const post1 = await prisma.communityPost.create({
    data: {
      ...
      content:
        "I have a spare bedroom in Pacific Grove available for anyone displaced by the Carmel Valley fire. Pet-friendly, can accommodate 2-3 people. DM me for details.",
  ...
  const post2 = await prisma.communityPost.create({
    data: {
      ...
      content:
        "Orange tabby named Whiskers, wearing blue collar. Last seen near River Road before evacuation. Please contact me if spotted.",
  ...
  const post4 = await prisma.communityPost.create({
    data: {
      ...
      content:
        "Just want to express gratitude to all the firefighters, EMTs, and volunteers working around the clock. You are heroes.",
      postType: "THANK_YOU",
      reviewStatus: "AUTO_APPROVED",
      postStatus: "PUBLISHED",
      upvoteCount: 234,
  ...
  const evacAlert = await prisma.alert.create({
    data: {
      ...
      message:
        "Mandatory evacuation order issued for Carmel Valley Zone 4 due to approaching wildfire. Leave immediately via Carmel Valley Road westbound. Shelter available at Carmel Middle School.",
      alertType: "EMERGENCY",
  ...
  const warningAlert = await prisma.alert.create({
    data: {
      ...
      message:
        "Evacuation WARNING for Carmel Valley Zone 5. Be prepared to leave on short notice. Gather essential items and important documents now.",
      alertType: "WARNING",
      source: "LOCAL_GOVERNMENT",
      severity: "MODERATE",
  ...
  const floodAlert = await prisma.alert.create({
    data: {
      ...
      message:
        "A Flood Watch is in effect for the Salinas River Basin from Wednesday evening through Friday morning. 2-4 inches of rain expected.",
      alertType: "WATCH",
      source: "NWS",
      severity: "MODERATE",
  ...
  const pspsAlert = await prisma.alert.create({
    data: {
      ...
      message:
        "PG&E has initiated a Public Safety Power Shutoff affecting Pacific Grove. Estimated restoration: 10:00 PM tonight. Prepare for extended outage.",
      alertType: "ADVISORY",
      source: "LOCAL_GOVERNMENT",
      severity: "MINOR",
