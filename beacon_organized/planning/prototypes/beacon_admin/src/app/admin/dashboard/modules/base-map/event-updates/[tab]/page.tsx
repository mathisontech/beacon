...
export default function EventUpdateTab({ params }: { params: Promise<{ tab: string }> }) {
  const { tab } = use(params);
  const sub = baseMapSubmodules.find(s => s.key === 'event-updates');
  ...
  return (
    <div>
      <p style={{ fontSize: 12, color: '#9ca3af', marginBottom: 4 }}>Event-Triggered Updates</p>
