...
import WildfireLayer from './WildfireLayer.jsx'
...
function InteractiveOSMMap({
  ...
  route = null,
  destination = null,
  enableWildfireLayer = true,
  wildfireSearchRadiusKm = 100,
  firmsApiKey = null
}) {
  ...
  const [showWildfires, setShowWildfires] = useState(enableWildfireLayer)
  const [showSpreadPredictions, setShowSpreadPredictions] = useState(true)
  ...
  return (
    <div className={`interactive-osm-map ${className} ${emergencyMode ? 'emergency-mode' : ''}`}>
      ...
      >
        ...
        {/* Wildfire layer with real-time NASA FIRMS data + spread predictions */}
        <WildfireLayer
          userLocation={userLocation}
          searchRadiusKm={wildfireSearchRadiusKm}
          showSpreadPredictions={showSpreadPredictions}
          predictionHours={predictionHours}
          enabled={showWildfires}
      ...
      <div className="map-controls">
        ...
        {/* Wildfire layer controls */}
        {enableWildfireLayer && (
          <div className="wildfire-controls">
            <button
              onClick={() => setShowWildfires(!showWildfires)}
              className={`wildfire-toggle-btn ${showWildfires ? 'active' : ''}`}
              title={showWildfires ? "Hide wildfire layer" : "Show wildfire layer"}
            >
              🔥
            </button>

            {showWildfires && (
              <div className="wildfire-options">
                <button
                  onClick={() => setShowSpreadPredictions(!showSpreadPredictions)}
                  className={`prediction-toggle-btn ${showSpreadPredictions ? 'active' : ''}`}
                  title={showSpreadPredictions ? "Hide spread predictions" : "Show spread predictions"}
                  style={{ fontSize: '11px', padding: '4px 8px' }}
                >
                  {showSpreadPredictions ? '📊 Predictions ON' : '📊 Predictions OFF'}
                </button>

                {showSpreadPredictions && (
                  <select
                    value={predictionHours}
                    onChange={(e) => setPredictionHours(Number(e.target.value))}
                    className="prediction-hours-select"
